...
def _redirect_url(request):
    ...
    next_one = request.GET.get(REDIRECT_FIELD_NAME)
    if not next_one:
        if settings.CAS_IGNORE_REFERER:
            next_one = settings.CAS_REDIRECT_URL
        else:
            next_one = request.META.get('HTTP_REFERER', settings.CAS_REDIRECT_URL)
        prefix = (('http://', 'https://')[request.is_secure()] +
                  HttpRequest.get_host(request))
        if next_one.startswith(prefix):
            next_one = next_one[len(prefix):]
    return next_one
